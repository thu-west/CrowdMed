...
function log_result(result) {
	axios.post('http://127.0.0.1:5000/log_base', result)
...
module.exports = async function(callback) {
	...
	for (idx = 1; idx < 10; idx++) {
		...
		for (counter = 0; counter < 500; counter++) {
			...
			log_result(await patient_SC[counter].newPermission(doctors[idx], "generic query string", {from: patients[counter]}));
