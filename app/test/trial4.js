...
function log_result(result) {
	axios.post('http://127.0.0.1:5000/log_1', result)
...
module.exports = async function(callback) {
	...
	log_result(await patientA_SC.newPermissionGroup("generic query", {from: patientA}));
	try {
		log_result(await patientA_SC.newPermission(accounts[3], 0, {from: patientA}));
	} catch (err) {
		console.log(err);
	}
	...
	try {
		log_result(await patientA_SC.newPermission(accounts[2], 0, {from: patientA}));
	} catch (err) {
		console.log(err);
	}
	...
	for (counter = 30; counter < 40; counter++) {
		qstring = "select where id=A" + counter.toString();
		hash = "pAdA_" + counter.toString();
		log_result(patientA_SC.newRecord(qstring, hash, {from: doctorA}));
	}

	// Step 11: DoctorB requests patientA data
	log_result(patientA_SC.transferData({from: doctorB}));
