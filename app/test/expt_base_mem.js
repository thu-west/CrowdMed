...
function log_result(result) {
	axios.post('http://127.0.0.1:5000/log_base_mem', result)
...
module.exports = async function(callback) {
	...
	// Step 2A: Patient - Doctor0 relationship
	for (counter = 0; counter < 500; counter++) {
		log_result(await patient_SC[counter].newPPR({from: doctors[0]}));
		log_result(await patient_SC[counter].newPermissionGroup(doctors[0], "generic query string", {from: patients[counter]}));
	}
	...
	for (idx = 1; idx < 10; idx++) {
		...
		for (counter = 0; counter < 500; counter++) {
			...
			log_result(await patient_SC[counter].newPermissionGroup(doctors[idx], "generic query string", {from: patients[counter]}));
